...
async function sendResend(env:Env,to:string,subject:string,message:string,actionUrl?:string){
  ...
  try{const r=await fetch('https://api.resend.com/emails',{method:'POST',headers:{authorization:`Bearer ${env.RESEND_API_KEY}`,'content-type':'application/json'},body:JSON.stringify({from:env.EMAIL_FROM,to:[to],subject,html})});if(!r.ok)return{ok:false,error:(await r.text().catch(()=>String(r.status))).slice(0,500)};return{ok:true,error:''}}catch(error){return{ok:false,error:error instanceof Error?error.message:String(error)}}
}
async function deliver(env:Env,patientId:number,appointmentId:number|null,kind:string,message:string,dedupeKey:string,payload:Record<string,unknown>={}){
  const patient=await env.DB.prepare(`SELECT email FROM patients WHERE id=?`).bind(patientId).first<any>();if(!patient?.email)return false
  const id=crypto.randomUUID(),key=`${dedupeKey}:email`,inserted=await env.DB.prepare(`INSERT OR IGNORE INTO patient_notifications(id,patient_id,appointment_id,kind,channel,status,message,payload_json,dedupe_key,retry_count,last_attempt_at) VALUES(?,?,?,?,?,'sending',?,?,?,?,CURRENT_TIMESTAMP)`).bind(id,patientId,appointmentId,kind,'email',message,JSON.stringify(payload),key,1).run();if(!Number(inserted.meta.changes||0))return false
  ...
  await env.DB.prepare(`UPDATE patient_notifications SET status='failed',error_message=?,next_retry_at=? WHERE id=?`).bind(result.error,nextRetryAt(1),id).run();return false
}
async function notificationAlreadyExists(env:Env,appointmentId:number,kind:string){const row=await env.DB.prepare(`SELECT id FROM patient_notifications WHERE appointment_id=? AND kind=? AND channel IN ('internal','email') LIMIT 1`).bind(appointmentId,kind).first<any>();return Boolean(row?.id)}
export async function sendPatientEventEmail(env:Env,patientId:number,appointmentId:number|null,kind:string,message:string,dedupeKey:string){return deliver(env,patientId,appointmentId,kind,message,dedupeKey,{action_url:`${env.APP_ORIGIN||''}/paciente`})}
export async function sendReservationCreatedEmail(env:Env,appointmentId:number){
  const row=await env.DB.prepare(`SELECT a.id,a.patient_id,a.reserved_until,a.payment_deadline_at,a.reservation_kind,av.starts_at FROM appointments a JOIN availability av ON av.id=a.availability_id WHERE a.id=?`).bind(appointmentId).first<any>();if(!row)return false
  const deadline=row.payment_deadline_at||row.reserved_until
  const message=row.reservation_kind==='recurring'?`Sua próxima consulta está reservada para ${dateLabel(row.starts_at)} às ${timeLabel(row.starts_at)}.${deadline?` O pagamento deve ser realizado até ${dateTimeLabel(deadline)}.`:''}`:`Sua reserva foi realizada para ${dateLabel(row.starts_at)} às ${timeLabel(row.starts_at)}.${deadline?` Conclua o pagamento até ${dateTimeLabel(deadline)} para confirmar o horário.`:''}`
...
async function mirrorInternalNotifications(env:Env){const rows=await env.DB.prepare(`SELECT n.id,n.patient_id,n.appointment_id,n.kind,n.message FROM patient_notifications n WHERE n.channel='internal' AND n.kind IN ('payment_reminder','payment_final','reservation_expired','rescheduled','professional_cancelled') ORDER BY n.created_at DESC LIMIT 100`).all<any>();for(const row of rows.results||[])await deliver(env,Number(row.patient_id),row.appointment_id==null?null:Number(row.appointment_id),String(row.kind),String(row.message),`internal:${row.id}`,{action_url:`${env.APP_ORIGIN||''}/paciente`})}
...
async function reliablePaymentReminders(env:Env){
  const now=Date.now(),lower=new Date(now+24*3600000).toISOString(),upper=new Date(now+48*3600000).toISOString()
  const rows=await env.DB.prepare(`SELECT a.id,a.patient_id,a.payment_deadline_at,a.reserved_until,a.reservation_kind,av.starts_at FROM appointments a JOIN availability av ON av.id=a.availability_id WHERE a.status='pending_payment' AND av.starts_at>? AND av.starts_at<=? ORDER BY av.starts_at`).bind(lower,upper).all<any>()
  for(const row of rows.results||[]){
    const deadlineRaw=row.payment_deadline_at||row.reserved_until;if(!deadlineRaw||new Date(deadlineRaw).getTime()<=now)continue
    if(await notificationAlreadyExists(env,Number(row.id),'payment_reminder'))continue
    const deadline=dateTimeLabel(deadlineRaw),message=`Lembrete: sua sessão está reservada para ${dateLabel(row.starts_at)} às ${timeLabel(row.starts_at)}. O pagamento deve ser confirmado até ${deadline}, limite de 24 horas antes da sessão.`
    await deliver(env,Number(row.patient_id),Number(row.id),'payment_reminder',message,`auto-pay48:${row.id}`,{action_url:`${env.APP_ORIGIN||''}/paciente`})
  }
}
async function reliableAppointmentReminders(env:Env){
  const now=Date.now(),from=new Date(now+30*60000).toISOString(),to=new Date(now+24*3600000).toISOString(),rows=await env.DB.prepare(`SELECT a.id,a.patient_id,av.starts_at FROM appointments a JOIN availability av ON av.id=a.availability_id WHERE a.status='confirmed' AND av.starts_at>=? AND av.starts_at<=? ORDER BY av.starts_at`).bind(from,to).all<any>()
  for(const row of rows.results||[]){if(await notificationAlreadyExists(env,Number(row.id),'appointment_reminder'))continue;const message=`Lembrete: sua consulta está confirmada para ${dateLabel(row.starts_at)} às ${timeLabel(row.starts_at)}.`;await deliver(env,Number(row.patient_id),Number(row.id),'appointment_reminder',message,`appt24:${row.id}`,{action_url:`${env.APP_ORIGIN||''}/paciente`})}
}
