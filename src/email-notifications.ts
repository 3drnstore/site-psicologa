...
async function reliableAppointmentReminders(env:Env){
  ...
  const lower=new Date(now+(23.5*3600000)).toISOString()
  const upper=new Date(now+(24*3600000)).toISOString()
  const rows=await env.DB.prepare(`SELECT a.id,a.patient_id,av.starts_at FROM appointments a JOIN availability av ON av.id=a.availability_id WHERE a.status='confirmed' AND av.starts_at>? AND av.starts_at<=? ORDER BY av.starts_at`).bind(lower,upper).all<any>()
  for(const row of rows.results||[]){
    if(await notificationAlreadyExists(env,Number(row.id),'appointment_reminder_24h'))continue
    const message=`Lembrete: sua consulta está confirmada para ${dateLabel(row.starts_at)} às ${timeLabel(row.starts_at)}.`
    await deliver(env,Number(row.patient_id),Number(row.id),'appointment_reminder_24h',message,`appt24-once:${row.id}`,{action_url:`${env.APP_ORIGIN||''}/paciente`})
  }
}
