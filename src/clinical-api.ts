...
export async function handleClinicalApi(request:Request,env:Env,path:string):Promise<Response|null>{
  ...
  if(isDetail){
    ...
    const recurrence=await env.DB.prepare('SELECT id,patient_id,cadence_days,active,source_appointment_id,manual_reference_at,created_at,updated_at FROM patient_recurrence WHERE patient_id=? LIMIT 1').bind(patientId).first<any>().catch(()=>null)
