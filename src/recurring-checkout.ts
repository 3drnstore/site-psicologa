...
export async function handleRecurringCheckout(request:Request,env:Env,path:string):Promise<Response|null>{
  ...
  const baseAmount=Math.max(0,Number(appointment.amount_cents||0));if(!baseAmount)return json({ok:false,message:'O valor desta sessão ainda não está configurado.'},409)
  ...
  if(existing&&(existing.pix_qr_code||existing.checkout_url))return json({ok:true,payment_id:existing.id,provider:existing.provider,pix_qr_code:existing.pix_qr_code,pix_copy_paste:existing.pix_copy_paste,checkout_url:existing.checkout_url,amount_cents:Number(existing.amount_cents),payment_deadline_at:deadline})
  const testMode=String(env.MERCADOPAGO_TEST_MODE||'').toLowerCase()==='true'
  const chargedAmount=method==='pix'&&testMode?5000:baseAmount
  const inserted=await env.DB.prepare(`INSERT INTO payments(appointment_id,provider,method,status,amount_cents) VALUES(?,?,?,'pending',?)`).bind(appointmentId,provider,method,chargedAmount).run();const paymentId=Number(inserted.meta.last_row_id)
  ...
  if(method==='pix'){
    ...
    const expirationMinutes=Math.max(30,Math.min(43200,remainingMinutes)),value=(chargedAmount/100).toFixed(2)
    const payload={type:'online',total_amount:value,external_reference:`recorrente-${appointmentId}-pagamento-${paymentId}`,processing_mode:'automatic',transactions:{payments:[{amount:value,payment_method:{id:'pix',type:'bank_transfer'},expiration_time:`PT${expirationMinutes}M`}]},payer:testMode?{email:'test_user_br@testuser.com',first_name:'APRO'}:{email:p.email,first_name:String(p.full_name||'Paciente').split(' ')[0]}}
    ...
    await env.DB.prepare(`UPDATE payments SET external_id=?,checkout_url=?,pix_qr_code=?,pix_copy_paste=?,raw_status=?,amount_cents=? WHERE id=?`).bind(external||null,ticket,qr,copy,String(data.status||'created'),chargedAmount,paymentId).run()
    await env.DB.prepare(`UPDATE appointments SET amount_cents=?,payment_method='pix',payment_provider='mercadopago',payment_external_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(chargedAmount,external||null,appointmentId).run()
  ...
  const r=await fetch('https://api.checkout.infinitepay.io/links',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({handle:env.INFINITEPAY_HANDLE,items:[{quantity:1,price:baseAmount,description:'Sessão psicológica recorrente'}],order_nsu:String(paymentId),redirect_url:`${origin}/?payment=return&provider=infinitepay`,webhook_url:`${origin}/api/payments/webhook/infinitepay`,customer:{name:p.full_name,email:p.email,phone_number:p.phone}})})
  ...
  return json({ok:true,payment_id:paymentId,provider:'infinitepay',checkout_url:checkout,amount_cents:baseAmount,payment_deadline_at:deadline})
}
