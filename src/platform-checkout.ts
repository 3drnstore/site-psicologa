...
export async function handlePlatformCheckout(request:Request,env:Env,path:string):Promise<Response|null>{
  ...
  if(existing&&((requested==='pix'&&(existing.pix_qr_code||existing.pix_copy_paste||existing.checkout_url))||(requested==='credit_card'&&existing.checkout_url))){
    return json({ok:true,payment_id:Number(existing.id),provider,checkout_url:existing.checkout_url,pix_qr_code:existing.pix_qr_code,pix_copy_paste:existing.pix_copy_paste,amount_cents:Number(existing.amount_cents)})
  }
  ...
  if(requested==='pix'){
    ...
    const testMode=String(env.MERCADOPAGO_TEST_MODE||'').toLowerCase()==='true',chargedAmount=testMode?5000:amount,holdMinutes=Math.max(30,Number(await setting(env,'hold_minutes','30'))||30),amountString=(chargedAmount/100).toFixed(2)
    await env.DB.prepare(`UPDATE appointments SET reserved_until=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(plusMinutes(holdMinutes),appointmentId).run()
    const payload={type:'online',total_amount:amountString,external_reference:`consulta-${appointmentId}-pagamento-${paymentId}`,processing_mode:'automatic',transactions:{payments:[{amount:amountString,payment_method:{id:'pix',type:'bank_transfer'},expiration_time:`PT${Math.max(30,Math.min(43200,holdMinutes))}M`}]},payer:testMode?{email:'test_user_br@testuser.com',first_name:'APRO'}:{email:p.email,first_name:String(p.full_name||'Paciente').split(' ')[0]}}
    ...
    return json({ok:true,payment_id:paymentId,provider:'mercadopago',pix_qr_code:qr,pix_copy_paste:copy,checkout_url:ticket,amount_cents:chargedAmount,test_mode:testMode})
  }
