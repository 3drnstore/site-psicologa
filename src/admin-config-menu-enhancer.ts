...
function ensureMenu(){
  ...
  if(!parent.dataset.configRouteBound){parent.dataset.configRouteBound='1';parent.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();navigate('pricing')},true)}
...
function navigate(page:ConfigPage,replace=false){
  const url=routes[page];if(location.pathname!==url){history[replace?'replaceState':'pushState']({},'',url)}
  localStorage.setItem('psicogestao.admin.view','configuracoes')
  render(page)
  window.dispatchEvent(new CustomEvent('psicogestao:config-route',{detail:{page}}))
}

function render(page:ConfigPage){
  const button=configButton();const menu=ensureMenu();if(!button||!menu)return
  setConfigActive();setHeader(page);menu.hidden=false
  menu.querySelectorAll<HTMLElement>('[data-config-page]').forEach(el=>el.classList.toggle('active',el.dataset.configPage===page))
  document.documentElement.dataset.adminConfigPage=page
}

function scan(){
  const menu=ensureMenu();if(!menu)return
  const page=pageFromPath()
  if(page){render(page);return}
  const active=configButton()?.classList.contains('active')===true
  menu.hidden=!active
  if(active)navigate('pricing',true)
  else delete document.documentElement.dataset.adminConfigPage
}

export function installAdminConfigMenuEnhancer(){
  let timer:number|undefined
  const schedule=()=>{if(timer)clearTimeout(timer);timer=window.setTimeout(()=>{timer=undefined;scan()},80)}
  schedule();[180,450,900].forEach(ms=>window.setTimeout(scan,ms))
  const root=document.getElementById('root');if(root)new MutationObserver(schedule).observe(root,{childList:true,subtree:true})
  window.addEventListener('popstate',schedule);window.addEventListener('pageshow',schedule)
}
