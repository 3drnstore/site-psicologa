...
function navigate(page:ConfigPage,replace=false){const url=routes[page];if(location.pathname!==url)history[replace?'replaceState':'pushState']({},'',url);localStorage.setItem('psicogestao.admin.view','configuracoes');clearForeignCustomView();render(page);window.dispatchEvent(new CustomEvent('psicogestao:config-route',{detail:{page}}))}
function render(page:ConfigPage){const button=configButton(),menu=ensureMenu();if(!button||!menu)return;clearForeignCustomView();setConfigActive();setHeader(page);menu.hidden=false;menu.querySelectorAll<HTMLElement>('[data-config-page]').forEach(el=>el.classList.toggle('active',el.dataset.configPage===page));document.documentElement.dataset.adminConfigPage=page}
function configContentMounted(){return Boolean(document.querySelector('.settings-panel'))&&!document.querySelector('.admin-custom-view')&&!document.querySelector('.admin-main.admin-custom-mode')}
function ensureReactConfigMounted(){const button=configButton();if(!button||activating)return;if(configContentMounted())return;activating=true;clearForeignCustomView();button.click();window.setTimeout(()=>{activating=false;const page=pageFromPath();if(page){clearForeignCustomView();render(page);window.dispatchEvent(new CustomEvent('psicogestao:config-route',{detail:{page}}))}},100)}
...
export function installAdminConfigMenuEnhancer(){let timer:number|undefined;const schedule=()=>{if(timer)clearTimeout(timer);timer=window.setTimeout(()=>{timer=undefined;scan()},80)};schedule();[120,260,500,900,1400].forEach(ms=>window.setTimeout(scan,ms));const root=document.getElementById('root');if(root)new MutationObserver(schedule).observe(root,{childList:true,subtree:true});window.addEventListener('popstate',schedule);window.addEventListener('pageshow',schedule)}
