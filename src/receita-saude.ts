...
async function ensureStorage(env:Env){
  ...
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS receita_saude_receipts(
  ...
  await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_receita_saude_status ON receita_saude_receipts(status,payment_date)`).run()
  storageReady=true
}
...
async function syncPending(env:Env){
  ...
  await env.DB.prepare(`
    ...
    SELECT a.id,a.patient_id,'pending',a.paid_at,a.amount_cents
    FROM appointments a
...
export async function handleReceitaSaude(request:Request,env:Env,path:string):Promise<Response|null>{
  ...
  try{
    ...
    console.error('Receita Saúde API error:',error instanceof Error?error.message:String(error))
    return json({ok:false,message:'Não foi possível carregar o controle do Receita Saúde. Tente novamente em alguns instantes.'},500)
  }
}
