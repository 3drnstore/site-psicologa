...
async function recurringReminders(env:Env){const rows=await env.DB.prepare(`SELECT a.id,a.patient_id,a.reserved_until,a.payment_deadline_at,av.starts_at FROM appointments a JOIN availability av ON av.id=a.availability_id WHERE a.status='pending_payment' AND a.reservation_kind='recurring' AND av.starts_at>?`).bind(nowIso()).all<any>(),origin=env.APP_ORIGIN||'';for(const row of rows.results||[]){const days=daysUntil(row.starts_at),deadlineValue=row.payment_deadline_at||row.reserved_until,deadline=deadlineValue?new Intl.DateTimeFormat('pt-BR',{timeZone:TZ,day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date(deadlineValue)):'';if(days===3)await queueNotification(env,row.patient_id,row.id,'payment_reminder',`Sua próxima sessão está reservada para ${ptDate(row.starts_at)} às ${ptTime(row.starts_at)}. Confirme o pagamento até ${deadline}.`,[ptDate(row.starts_at),ptTime(row.starts_at),deadline,`${origin}/paciente`],`pay3d:${row.id}`);if(days===2)await queueNotification(env,row.patient_id,row.id,'payment_final',`Último lembrete: confirme sua sessão de ${ptDate(row.starts_at)} às ${ptTime(row.starts_at)} até ${deadline}.`,[ptDate(row.starts_at),ptTime(row.starts_at),deadline,`${origin}/paciente`],`pay2d:${row.id}`)}}
