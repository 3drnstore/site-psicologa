...
export async function handleAdminPatientsV2(request:Request,env:Env,path:string):Promise<Response|null>{
    ...
    try{
      ...
      if(hasAppointments&&hasAvailability){
        const result=await env.DB.prepare(`
          ...
          GROUP BY p.id,p.full_name,p.birth_date,p.cpf,p.phone,p.email,p.created_at
          ORDER BY p.full_name COLLATE NOCASE
        `).all<any>()
        return json({ok:true,patients:result.results||[]})
      }
      const result=await env.DB.prepare(`SELECT id,full_name,birth_date,cpf,phone,email,created_at,0 AS appointment_count,NULL AS last_appointment_at FROM patients ORDER BY full_name COLLATE NOCASE`).all<any>()
      return json({ok:true,patients:result.results||[]})
    }catch(error){
      console.error('Admin patients list error:',error)
      return json({ok:false,message:'Não foi possível carregar os pacientes cadastrados.'},500)
    }
  }
  ...
  if(match&&request.method==='GET'){
    ...
    const patient=await env.DB.prepare(`SELECT id,full_name,birth_date,cpf,phone,email,created_at FROM patients WHERE id=?`).bind(id).first<any>()
    ...
    if(await tableExists(env,'appointments')){
      const result=await env.DB.prepare(`SELECT a.id,a.status,a.amount_cents,a.paid_at,av.starts_at,av.ends_at FROM appointments a LEFT JOIN availability av ON av.id=a.availability_id WHERE a.patient_id=? ORDER BY av.starts_at DESC`).bind(id).all<any>()
      appointments=result.results||[]
    }
    if(await tableExists(env,'clinical_notes')){
      const result=await env.DB.prepare(`SELECT id,appointment_id,session_date,note_text,created_at,updated_at FROM clinical_notes WHERE patient_id=? ORDER BY session_date DESC,created_at DESC`).bind(id).all<any>()
      clinical_notes=result.results||[]
    }
