...
async function reconcilePatient(env:Env,id:number){
  const rows=await env.DB.prepare(`SELECT a.id FROM appointments a JOIN availability av ON av.id=a.availability_id WHERE a.patient_id=? AND a.status='confirmed' ORDER BY av.starts_at DESC LIMIT 1`).bind(id).all<any>()
