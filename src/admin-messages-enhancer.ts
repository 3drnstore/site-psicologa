...
async function deleteMessage(id:number){const r=await fetch('/api/contact?admin=1',{method:'DELETE',credentials:'include',headers:{'content-type':'application/json'},body:JSON.stringify({id})});const d=await r.json().catch(()=>({})) as any;if(!r.ok)throw new Error(d.message||'Não foi possível excluir a mensagem.')}
...
async function renderMessages(button:HTMLButtonElement){
  ...
  try{
    ...
    host.innerHTML=`<div class="admin-messages-summary"><span><strong>${unread}</strong> nova(s)</span><span>${messages.length} mensagem(ns)</span></div><div class="admin-messages-list">${messages.length?messages.map(m=>`<article class="admin-message-card ${m.status==='new'?'is-new':''}" data-message-id="${m.id}"><div class="admin-message-head"><div><strong>${esc(m.name)}</strong><span>${esc(dateTime(m.created_at))}</span></div><span class="admin-message-status">${m.status==='new'?'Nova':'Lida'}</span></div><div class="admin-message-contact"><a href="mailto:${esc(m.email)}">${esc(m.email)}</a><a href="tel:${esc(m.phone)}">${esc(m.phone)}</a></div><p>${esc(m.message)}</p><div class="admin-message-actions"><button type="button" class="secondary-button" data-message-toggle="${m.status==='new'?'read':'new'}">${m.status==='new'?'Marcar como lida':'Marcar como nova'}</button><button type="button" class="secondary-button admin-message-delete" data-message-delete>Excluir</button></div></article>`).join(''):'<div class="admin-dashboard-empty">Nenhuma mensagem recebida.</div>'}</div>`
    ...
    host.querySelectorAll<HTMLButtonElement>('[data-message-delete]').forEach(action=>action.addEventListener('click',async()=>{const card=action.closest<HTMLElement>('[data-message-id]');if(!card)return;if(!confirm('Excluir esta mensagem permanentemente?'))return;action.disabled=true;try{await deleteMessage(Number(card.dataset.messageId));await renderMessages(button)}catch(e){action.disabled=false;alert(e instanceof Error?e.message:'Não foi possível excluir a mensagem.')}}))
