...
const contactActions=(a:Appointment)=>{
  ...
  return `<span class="admin-consult-contact">${phone?`<a href="https://wa.me/${phone}" target="_blank" rel="noopener noreferrer">WhatsApp</a>`:''}${email?`<a href="mailto:${esc(email)}">E-mail</a>`:''}<button type="button" class="admin-consult-reschedule" data-reschedule-id="${a.id}">Reagendar</button><button type="button" class="admin-consult-cancel" data-cancel-id="${a.id}">Cancelar consulta</button></span>`
}
