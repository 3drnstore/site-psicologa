...
export type AdminConsultation={id:number;status:string;starts_at:string;ends_at:string;full_name:string;email:string;phone?:string|null}
...
const contactActions=(a:AdminConsultation)=>{
...
async function appointments(){const r=await fetch('/api/admin/appointments',{credentials:'include',cache:'no-store'});const d=await r.json().catch(()=>({})) as any;if(!r.ok)throw new Error(d.message||'Não foi possível carregar as consultas.');return (d.appointments||[]) as AdminConsultation[]}

export function renderAdminConsultations(host:HTMLElement,all:AdminConsultation[]){
  host.dataset.consultV2='1';host.classList.add('admin-consultations-v2')
...
async function enhance(){
  ...
  try{
    ...
    lastHost=host;renderAdminConsultations(host,all)
